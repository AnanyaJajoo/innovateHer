...
export const recomputeDomainReputation = async (domain: string) => {
  ...
  try {
    ...
    const scam = reports.filter((r) => r.reportType === "scam" || r.type === "scam").length;
    const fp = reports.filter(
      (r) => r.reportType === "false_positive" || r.type === "false_positive"
    ).length;

    const raw = scam - fp;
    const aggregateRiskScore = clamp(50 + 15 * raw, 0, 100);
